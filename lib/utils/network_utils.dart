...
class NetworkUtils {
  ...
  static Future<http.Response> optimizedGet(
    ...
    try {
      if (kDebugMode && Platform.isAndroid) {
        print('NetworkUtils: Android优化请求 - ${uri.toString()}');
        print('NetworkUtils: 超时设置 - ${timeout.inSeconds}秒');
      }
      
      final response = await client.get(uri, headers: headers).timeout(timeout);
      
      if (kDebugMode && Platform.isAndroid) {
        print('NetworkUtils: 请求完成 - 状态码: ${response.statusCode}');
      }
  ...
  static Future<http.Response> optimizedPost(
    ...
    try {
      if (kDebugMode && Platform.isAndroid) {
        print('NetworkUtils: Android优化POST请求 - ${uri.toString()}');
      }
  ...
  static Future<http.Response> directPost(
    ...
    try {
      if (kDebugMode) {
        print('NetworkUtils: 标准POST请求 - ${uri.toString()}');
        print('NetworkUtils: 允许通过代理连接');
      }
  ...
  static Future<http.Response> directDelete(
    ...
    try {
      if (kDebugMode) {
        print('NetworkUtils: 标准DELETE请求 - ${uri.toString()}');
        print('NetworkUtils: 允许通过代理连接');
      }
