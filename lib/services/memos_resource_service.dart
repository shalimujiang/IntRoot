...
class MemosResourceService {
  ...
  Future<Map<String, dynamic>> uploadImage(File imageFile) async {
    try {
      print('MemosResourceService: 开始上传图片 - ${imageFile.path}');
      ...
      print('MemosResourceService: 发送上传请求到 $uri');
      
      // 发送请求
      final streamedResponse = await request.send();
      final response = await http.Response.fromStream(streamedResponse);
      
      print('MemosResourceService: 收到响应，状态码: ${response.statusCode}');
      
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        print('MemosResourceService: 上传成功 - ${data}');
        ...
        print('MemosResourceService: 上传失败: ${response.statusCode} - ${response.body}');
        throw Exception('上传失败: ${response.statusCode}');
      }
    } catch (e) {
      print('MemosResourceService: 上传图片时发生错误: $e');
  ...
  Future<List<Map<String, dynamic>>> uploadImages(List<File> imageFiles) async {
    ...
    for (int i = 0; i < imageFiles.length; i++) {
      try {
        print('MemosResourceService: 上传第${i + 1}/${imageFiles.length}张图片');
        final result = await uploadImage(imageFiles[i]);
        results.add(result);
      } catch (e) {
        print('MemosResourceService: 第${i + 1}张图片上传失败: $e');
        results.add({
          'success': false,
          'error': e.toString(),
  ...
  /// 生成图片的Markdown代码
  static String generateImageMarkdown(Map<String, dynamic> uploadResult, {String alt = '图片'}) {
