...
class DatabaseService {
  ...
  Future<Database> _initDatabase() async {
    ...
    return await openDatabase(
      path,
      version: 4, // 增加数据库版本以支持reminder_time列
      ...
      onUpgrade: (db, oldVersion, newVersion) async {
        ...
        if (oldVersion < 2) {
          await db.execute('ALTER TABLE notes ADD COLUMN resourceList TEXT DEFAULT "[]"');
        }
        if (oldVersion < 3) {
          await db.execute('ALTER TABLE notes ADD COLUMN relations TEXT DEFAULT "[]"');
        }
        if (oldVersion < 4) {
