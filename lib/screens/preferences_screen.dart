...
class _PreferencesScreenState extends State<PreferencesScreen> {
  ...
  // 更新自动登录设置
  void _updateAutoLogin(AppProvider appProvider, bool value) async {
    // 如果要开启自动登录，必须先开启记住密码
    if (value && !appProvider.appConfig.rememberLogin) {
      ...
      // 显示提示信息
      if (mounted) {
        SnackBarUtils.showInfo(context, '已同时开启记住密码功能');
      }
    } else {
      final newConfig = appProvider.appConfig.copyWith(
        autoLogin: value,
  ...
  // 构建默认笔记可见性选择器
  Widget _buildDefaultVisibilitySelector(BuildContext context, AppProvider appProvider, AppConfig appConfig) {
    final isDarkMode = Theme.of(context).brightness == Brightness.dark;
    final primaryColorWithOpacity = AppTheme.primaryColor.withOpacity(0.1);
    final iconColor = isDarkMode ? AppTheme.primaryLightColor : AppTheme.primaryColor;
    final textColor = isDarkMode ? AppTheme.darkTextPrimaryColor : AppTheme.textPrimaryColor;
            ...
            child: Column(
              ...
              children: [
                ...
                Container(
                  ...
                  child: DropdownButton<String>(
                    value: appConfig.defaultNoteVisibility,
                    isExpanded: true,
                    icon: Icon(Icons.arrow_drop_down, color: iconColor),
                    underline: Container(), // 移除下划线
                    dropdownColor: dropdownColor,
                    items: [
                      _buildVisibilityDropdownItem('私有', AppConfig.VISIBILITY_PRIVATE, isDarkMode, iconColor),
                      _buildVisibilityDropdownItem('公开', AppConfig.VISIBILITY_PUBLIC, isDarkMode, iconColor),
                    ],
                    onChanged: (value) {
                      if (value != null) {
                        _updateDefaultVisibility(appProvider, value);
                      }
                    },
                  ),
                ),
  ...
  // 构建可见性下拉项
  DropdownMenuItem<String> _buildVisibilityDropdownItem(String label, String value, bool isDarkMode, Color iconColor) {
    final isSelected = value == Provider.of<AppProvider>(context).appConfig.defaultNoteVisibility;
    final selectedColor = isDarkMode ? Colors.white : AppTheme.primaryColor;
    ...
    return DropdownMenuItem<String>(
      ...
      child: Row(
        children: [
          Icon(
            value == AppConfig.VISIBILITY_PRIVATE ? Icons.lock_outline : Icons.public_outlined,
            size: 18,
            color: isSelected ? iconColor : (isDarkMode ? Colors.grey[400] : Colors.grey[600]),
          ),
              ...
              children: [
                Text(
                  ...
                  style: TextStyle(
                    fontSize: 16,
                    fontWeight: FontWeight.w500,
                    color: isSelected ? (isDarkMode ? Colors.white : AppTheme.textPrimaryColor) : null,
                  ),
                ),
                Text(
                  value == AppConfig.VISIBILITY_PRIVATE ? '仅自己可见' : '任何人都可通过链接访问',
                  style: TextStyle(
                    fontSize: 12,
  ...
  // 更新默认笔记可见性
  void _updateDefaultVisibility(AppProvider appProvider, String visibility) async {
    final newConfig = appProvider.appConfig.copyWith(defaultNoteVisibility: visibility);
    await appProvider.updateConfig(newConfig);
    setState(() {});
    
    // 显示确认消息
    if (mounted) {
      final visibilityName = visibility == AppConfig.VISIBILITY_PRIVATE ? '私有' : '公开';
